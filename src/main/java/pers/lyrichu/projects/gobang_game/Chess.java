...
import pers.lyrichu.projects.gobang_game.Constants.ChessFirst;

import static pers.lyrichu.projects.gobang_game.Constants.CHESSBOARD_SIZE;

//下棋业务核心类，与界面棋盘对应，业务放在这里，可以和界面代码分离
public class Chess {

  //先手信息,默认人类先手
  public static ChessFirst chessFirst = ChessFirst.HUMAN;
  //界面棋盘,存储每格状态,有3种状态:机器落子,人类落子,未落子
  public static ChessCellStatus[][] chessboard = new ChessCellStatus[CHESSBOARD_SIZE][CHESSBOARD_SIZE];
  //每个位置得分
  private int[][] score = new int[CHESSBOARD_SIZE][CHESSBOARD_SIZE];

  public void init() {
    chessFirst = ChessFirst.HUMAN;//默认人类先手
    for(int i = 0; i  < CHESSBOARD_SIZE; i++) {
      for(int j = 0; j < CHESSBOARD_SIZE; j++) {
        chessboard[i][j] = ChessCellStatus.EMPTY;
        score[i][j] = 0;
      }
    }
  }
  ...
  public void addChessman(int x, int y, ChessCellStatus cellStatus) {
    chessboard[x][y] = cellStatus;
  }

  //判断落子位置是否合法
  public boolean isLegal(int x, int y) {
    if (x >=0 && x < CHESSBOARD_SIZE && y >= 0 && y < CHESSBOARD_SIZE
        && chessboard[x][y] == ChessCellStatus.EMPTY) {
      return true;
    }
    return false;
  }
  ...
  public boolean isWin(int x, int y, ChessCellStatus cellStatus) {
    int sum = 0;
    //判断横向左边
    for(int i = x - 1; i >= 0; i--) {
      if(chessboard[i][y] == cellStatus) {
        sum++;
      } else {
        break;
      }
  ...
  /**
   * 机器落子核心搜索算法，使用五元组评分算法
   * 算法思路：对15 X 15的 572个五元组分别评分，一个五元组的得分就是该五元组为其中每个位置贡献的分数,
   * 一个位置的分数就是其所在所有五元组分数之和。所有空位置中分数最高的那个位置就是落子位置。
   * reference:https://blog.csdn.net/u011587401/article/details/50877828
   * @return
   */
  public Location searchLocation() {
    ...
    for(int i = 0; i < CHESSBOARD_SIZE; i++) {
      for(int j = 0; j < 11; j++) {
        int k = j;
        while(k < j + 5) {
          if(chessboard[i][k] == ChessCellStatus.MACHINE) {
            machineChessmanNum++;
          }
          else if(chessboard[i][k] == ChessCellStatus.HUMAN) {
            humanChessmanNum++;
          }
          k++;
        }
    ...
    for(int i = 0; i < CHESSBOARD_SIZE; i++) {
      for(int j = 0; j < 11; j++){
        int k = j;
        while(k < j + 5){
          if(chessboard[k][i] == ChessCellStatus.MACHINE) {
            machineChessmanNum++;
          }
          else if(chessboard[k][i] == ChessCellStatus.HUMAN) {
            humanChessmanNum++;
          }
          k++;
        }
    ...
    for(int i = 14; i >= 4; i--) {
      for(int k = i, j = 0; j < CHESSBOARD_SIZE && k >= 0; j++, k--) {
        ...
        int n = j;
        while(m > k - 5 && k - 5 >= -1) {
          if(chessboard[m][n] == ChessCellStatus.MACHINE) {
            machineChessmanNum++;
          }
          else if(chessboard[m][n] == ChessCellStatus.HUMAN) {
            humanChessmanNum++;
          }
          m--;
          n++;
    ...
    for(int i = 1; i < CHESSBOARD_SIZE; i++) {
      for(int k = i, j = 14; j >= 0 && k < CHESSBOARD_SIZE; j--, k++) {
        int m = k;
        int n = j;
        while(m < k + 5 && k + 5 <= CHESSBOARD_SIZE) {
          if(chessboard[n][m] == ChessCellStatus.MACHINE) {
            machineChessmanNum++;
          }
          else if(chessboard[n][m] == ChessCellStatus.HUMAN) {
            humanChessmanNum++;
          }
          m++;
          n--;
    ...
    for(int i = 0; i < CHESSBOARD_SIZE; i++) {
      for(int j = 0; j < CHESSBOARD_SIZE; j++) {
        if(chessboard[i][j] == ChessCellStatus.EMPTY && score[i][j] > maxScore) {
          goalX = i;
          goalY = j;
          maxScore = score[i][j];
        }
    ...
    if(goalX != -1 && goalY != -1) {
      return new Location(goalX, goalY, ChessCellStatus.MACHINE);
    }
