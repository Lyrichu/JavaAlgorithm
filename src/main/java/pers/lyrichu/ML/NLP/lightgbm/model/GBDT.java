...
import pers.lyrichu.ML.NLP.lightgbm.function.ObjectiveFunction;
...
public class GBDT extends Boosting {
    ...
    public static final long serialVersionUID = -1459139427941842408L;
    ...
    public List<Double> predictRaw(SparseVector features, PredictionEarlyStopInstance early_stop) {
        ...
        for (int i = 0; i < num_iteration_for_pred_; ++i) {
            ...
            if (early_stop.getRoundPeriod() == early_stop_round_counter) {
                if (early_stop.getCallbackFunction().callback(output, num_tree_per_iteration_))
                    break;
